// author : Tommaso Costanzo
// problem : Circular RMQ http://codeforces.com/problemset/problem/52/C
...
int64_t range_min(std::vector<node>& tree, std::vector<int64_t>& lazy, int q_left, int q_right, int current) {
...
void inc(std::vector<node>& tree, std::vector<int64_t>& lazy, int q_left, int q_right, int value, int current) {
    ...
    if (left >= q_left && right <= q_right) {
        ...
        if (left != right) {
            ...
            if (LEFT(current) < tree.size()) lazy[LEFT(current)] += value;
            if (RIGHT(current) < tree.size()) lazy[RIGHT(current)] += value;
        }
